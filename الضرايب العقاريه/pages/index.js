...
export default function Home() {
  ...
  const doSearch = useCallback(async (q = query) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResults([]);
        setUnderstoodAs("");
        setErrorMessage(data.message || "لم أجد سؤالاً مطابقاً بدرجة كافية داخل قاعدة البيانات.");
      } else {
        setResults([{
          id: data.matchedQuestion,
          category: data.category || "Google Sheets",
  ...
  const handlePrint = (r) => {
    ...
    <h3>📜 الإجابة القانونية الرسمية</h3>
    <div class="answer">${safeAnswer}</div>
    ${safeExplanation ? `<div class="explain"><strong>💬 شرح مبسط:</strong><br>${safeExplanation}</div>` : ""}
  ...
  const S = {
    ...
    resultTitle: { fontSize:"0.95rem", fontWeight:800, marginBottom:"0.65rem", color:c.text, display:"flex", alignItems:"center", gap:"0.35rem" },
    answerCard: { background:c.ansBg, borderRadius:13, padding:"1rem", marginBottom:"0.9rem", whiteSpace:"pre-line", lineHeight:1.9, fontSize:"0.9rem", border:`1px solid ${c.border}` },
    explainCard: { background:dark?"rgba(99,102,241,0.1)":"rgba(99,102,241,0.08)", borderRadius:13, padding:"1rem", marginBottom:"0.9rem", whiteSpace:"pre-line", lineHeight:1.9, fontSize:"0.9rem", border:`1px solid ${c.border}` },
            ...
            <>
              {results.length === 0 ? (
                <div style={S.noRes}>
                  ...
                  <div style={{ fontSize:"1.05rem", fontWeight:700, marginBottom:"0.5rem" }}>لم يتم العثور على تطابق مناسب.</div>
                ...
                <>
                  ...
                  {/* Result cards */}
                  {results.map((r, idx) => (
                      <div key={r.id} style={{ animationDelay:`${idx * 0.1}s` }} className="rc">
                        ...
                        <div style={S.card} className="card-h">
                          <div style={S.resultTitle}>📜 الإجابة القانونية الرسمية</div>
                          <div style={S.answerCard}>{r.answer}</div>
                        </div>

                        {r.aiExplanation && (
                          <div style={S.card} className="card-h">
                            <div style={S.resultTitle}>💬 شرح مبسط</div>
